...
echo " "
/home/steam/steamcmd/steamcmd.sh +@sSteamCmdForcePlatformType windows +force_install_dir /mnt/vrising/server +login anonymous +app_update 1829350 validate +quit
echo "server: "`ls /mnt/vrising/server`
...
WINEPREFIX="$HOME/prefix64" xvfb-run --auto-servernum --server-args='-screen 0 640x480x24:32' wine64 /mnt/vrising/server/VRisingServer.exe -persistentDataPath $p -serverName "$SERVERNAME" -saveName "$WORLDNAME" -logFile "$p/VRisingServer.log" "$game_port" "$query_port" 2>&1
